...
let
  ...
  presetRequiredTools = lib.unique (lib.flatten (
    lib.mapAttrsToList 
      (_: preset: preset._module.args.requiredTools or []) 
  ...
  mcpConfigFile = pkgs.writeTextFile {
    ...
    text = builtins.toJSON {
      mcpServers = lib.mapAttrs (name: serverCfg:
        let
          isValidConfigValue = optName: optValue:
            optValue != null
  ...
  emacsConfigGenerator = {
    ...
    serverToConfig = name: cfg:
      let
        properties = lib.concatStringsSep " " (lib.filter (x: x != "") [
    ...
    lib.mkMerge [
      {
        scripts.gen-mcp-configurations = {
          exec = ''
            ...
            manage_config_file "$MCP_DEST_PATH" "$MCP_BACKUP_PATH" "$MCP_CONFIG_PATH" "${if cfg.enable then "true" else "false"}"

            # Manage .dir-locals.el based on supportEmacs option
            manage_config_file "$DIR_LOCALS_PATH" "$DIR_LOCALS_BACKUP_PATH" "${dirLocalsFile}" "${if cfg.enable && cfg.supportEmacs then "true" else "false"}"
